...
// sounds/README.md. space_invaders_plugin.c's render_audio() resamples from
// this rate to AUDIO_SAMPLE_RATE (src/platform/audio/audio_engine.h) at mix
// time (nearest-sample, see voice_advance() in space_invaders_plugin.c), so
// a file recorded/converted at any other rate still just plays back at the
// wrong pitch/speed - the two rates no longer have to match, but the file's
// actual rate has to match this constant.
